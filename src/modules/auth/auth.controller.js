...
export const signup = catchAsync(async (req, res, next) => {
  ...
  const link = `http://localhost:${process.env.PORT}/api/v1/auth/confirmEmail/${activationCode}`;
...
export const login = catchAsync(async (req, res, next) => {
  ...
  // generate token
  const token = jwt.sign({ id: user._id, role: "test", email: user.email }, process.env.TOKEN_KEY, { expiresIn: '2d' });
